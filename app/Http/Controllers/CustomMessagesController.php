...
class CustomMessagesController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if (Auth::user()->agent_immobiliers) { // Vérifiez si l'utilisateur est un agent immobilier
            // On récupère l'ID de l'agent immobilier connecté
            $agentId = Auth::user()->agent_immobiliers->id;

            $records = User::whereHas('locataires', function ($query) use ($agentId) {
                // Vérifie si le locataire est assigné à cet agent immobilier
                $query->where('agent_id', $agentId);
            })
                ->where('name', 'LIKE', "%{$input}%")
                ->paginate($request->per_page ?? $this->perPage);
        } else {
            // Recherche standard pour les autres utilisateurs
            $records = User::where('id', '!=', Auth::user()->id)
                ->where('name', 'LIKE', "%{$input}%")
                ->paginate($request->per_page ?? $this->perPage);
        }
